...
Так же перед запуском добавить папку `metabase` с докер файлом иначе будет ошибка при запуске сборки.
...
Так же запуск `docker-compose up -d` можно было делать в виртуальном окружении `venv`
